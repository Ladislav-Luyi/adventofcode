...
public class Main {
    ...
    static Pattern pattern = Pattern.compile("^(\\w+ \\w+): (\\w+): (\\w+), (\\w+): (\\w+), (\\w+): (\\w+)", Pattern.CASE_INSENSITIVE);
    static String input = """
    ...
    public static void main(String[] args) {
        List<Aunt> list = Arrays.stream(input.split("\n"))
                .map(Main::make)
                .toList();
        list.forEach(System.out::println);
        Aunt reference = new Aunt();
        reference.setChildren(3);
        reference.setCats(7);
        reference.setSamoyeds(2);
        reference.setPomeranians(3);
        reference.setAkitas(0);
        reference.setVizslas(0);
        reference.setGoldfish(5);
        reference.setTrees(3);
        reference.setCars(2);
        reference.setPerfumes(1);

        System.out.println("before filter  " + list.size());
        List<Aunt> result = list
                .stream()

//                .filter(aunt -> isMinusOne(aunt.getCats()) || reference.getCats() == aunt.getCats()) // part1
                .filter(aunt -> isMinusOne(aunt.getCats()) || reference.getCats() < aunt.getCats()) // part2
//                .filter(aunt -> isMinusOne(aunt.getTrees()) || reference.getTrees() == aunt.getTrees())    // part1
                .filter(aunt -> isMinusOne(aunt.getTrees()) || reference.getTrees() < aunt.getTrees())    // part2
//                .filter(aunt -> isMinusOne(aunt.getPomeranians()) || reference.getPomeranians() == aunt.getPomeranians()) // part1
                .filter(aunt -> isMinusOne(aunt.getPomeranians()) || reference.getPomeranians() > aunt.getPomeranians()) // part2
//                .filter(aunt -> isMinusOne(aunt.getGoldfish()) || reference.getGoldfish() == aunt.getGoldfish()) // part1
                .filter(aunt -> isMinusOne(aunt.getGoldfish()) || reference.getGoldfish() > aunt.getGoldfish()) // part2

                .filter(aunt -> isMinusOne(aunt.getChildren()) || reference.getChildren() == aunt.getChildren() )
                .filter(aunt -> isMinusOne(aunt.getSamoyeds()) || reference.getSamoyeds() == aunt.getSamoyeds())
                .filter(aunt -> isMinusOne(aunt.getAkitas()) || reference.getAkitas() == aunt.getAkitas())
...
//                .max(Comparator.comparing(Aunt::countRelevant)).stream() // part2
    ...
    private static boolean isMinusOne(int i) {
        return i == -1;
    }

    private static Aunt make(String s) {
        Matcher m = pattern.matcher(s);
        boolean b = m.find();
        Aunt aunt = new Aunt();
        aunt.setName(m.group(1));
        List<Pair> group = List.of( new Pair(m.group(2), m.group(3)) ,new Pair(m.group(4), m.group(5)), new Pair(m.group(6), m.group(7)));
        group.forEach(e -> addProperties(e.s, Integer.parseInt(e.i), aunt));

        return aunt;
    }

    private static void addProperties(String s, int i, Aunt aunt) {
        switch (s) {
            case "children":
                aunt.setChildren(i);
                break;
