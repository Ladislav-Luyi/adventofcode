...
private fun part1() {
    val hs = HashSet<Coordinates>();

    var currentCoordinate = Coordinates(0, 0);
    hs.add(currentCoordinate);
    for (lines in input.split("\n")) {
        for (s in lines.toCharArray()) {
            var row = currentCoordinate.row
            var column = currentCoordinate.column
            when (s) {
                '^' -> row--
                'v' -> row++
                '>' -> column++
            ...
            currentCoordinate = Coordinates(row, column)
            hs.add(currentCoordinate);
        }
    }
    println(hs.size)
...
private fun part2() {
    ...
    var currentCoordinateSanta = Coordinates(0, 0);
    var currentCoordinateRobot = Coordinates(0, 0);
    hs.add(currentCoordinateSanta);
    hs.add(currentCoordinateRobot)
    var row = 0
    var column = 0
    for (lines in input.split("\n")) {
        var isRobot = true;
        for (s in lines.toCharArray()) {
            if (isRobot){
                row = currentCoordinateRobot.row
                column = currentCoordinateRobot.column
                isRobot = false;
            }else{
                row = currentCoordinateSanta.row
                column = currentCoordinateSanta.column
                isRobot = true;
            }
            when (s) {
                '^' -> row--;
            ...
            if (!isRobot){
                currentCoordinateRobot = Coordinates(row, column)
                hs.add(currentCoordinateRobot)
            }else{
                currentCoordinateSanta = Coordinates(row, column)
                hs.add(currentCoordinateSanta);
            }
        }
    }
